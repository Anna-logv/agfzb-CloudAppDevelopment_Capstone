...
function main(params) {
    ...
    let result ={state:params.state}
    let entries={}
    if (params.state) {
      entries = getMatchingRecords(cloudant,'dealerships',{state:params.state});
    } else {
       entries = getAllRecords(cloudant,'dealerships');
    }
    return entries;
}
 ...
 function getMatchingRecords(cloudant,dbname, selector) {
     return new Promise((resolve, reject) => {
         cloudant.postFind({db:dbname,selector:selector})
                 .then((result)=>{
                   resolve({result:result.result.docs,state:selector.state});
                 })
                 .catch(err => {
                    console.log(err);
                     reject({ err: err });
